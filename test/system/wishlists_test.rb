require "application_system_test_case"

class WishlistsTest < ApplicationSystemTestCase
  setup do
    @wishlist = wishlists(:one)
  end

  test "visiting the index" do
    visit wishlists_url
    assert_selector "h1", text: "Wishlists"
  end

  test "creating a Wishlist" do
    visit wishlists_url
    click_on "New Wishlist"

    fill_in "Author", with: @wishlist.author_id
    fill_in "Expected release", with: @wishlist.expected_release
    fill_in "Image url", with: @wishlist.image_url
    fill_in "Martinus url", with: @wishlist.martinus_url
    fill_in "Name", with: @wishlist.name
    fill_in "Note", with: @wishlist.note
    fill_in "Pages", with: @wishlist.pages
    fill_in "Price", with: @wishlist.price
    fill_in "Publish year", with: @wishlist.publish_year
    fill_in "Publisher", with: @wishlist.publisher_id
    fill_in "User id", with: @wishlist.user_id_id
    click_on "Create Wishlist"

    assert_text "Wishlist was successfully created"
    click_on "Back"
  end

  test "updating a Wishlist" do
    visit wishlists_url
    click_on "Edit", match: :first

    fill_in "Author", with: @wishlist.author_id
    fill_in "Expected release", with: @wishlist.expected_release
    fill_in "Image url", with: @wishlist.image_url
    fill_in "Martinus url", with: @wishlist.martinus_url
    fill_in "Name", with: @wishlist.name
    fill_in "Note", with: @wishlist.note
    fill_in "Pages", with: @wishlist.pages
    fill_in "Price", with: @wishlist.price
    fill_in "Publish year", with: @wishlist.publish_year
    fill_in "Publisher", with: @wishlist.publisher_id
    fill_in "User id", with: @wishlist.user_id_id
    click_on "Update Wishlist"

    assert_text "Wishlist was successfully updated"
    click_on "Back"
  end

  test "destroying a Wishlist" do
    visit wishlists_url
    page.accept_confirm do
      click_on "Destroy", match: :first
    end

    assert_text "Wishlist was successfully destroyed"
  end
end
